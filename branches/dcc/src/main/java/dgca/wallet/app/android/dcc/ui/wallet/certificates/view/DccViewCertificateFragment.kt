...
import com.android.app.dcc.databinding.FragmentCertificateViewBinding
import dagger.hilt.android.AndroidEntryPoint
import dgca.wallet.app.android.dcc.model.getCertificateListData
import dgca.wallet.app.android.dcc.nfc.DCCApduService
import dgca.wallet.app.android.dcc.nfc.DCCApduService.Companion.NFC_NDEF_KEY
import dgca.wallet.app.android.dcc.nfc.showTurnOnNfcDialog
import dgca.wallet.app.android.dcc.ui.BindingFragment
import dgca.wallet.app.android.dcc.ui.wallet.qr.certificate.CertListAdapter
import timber.log.Timber
import javax.inject.Inject

@AndroidEntryPoint
class DccViewCertificateFragment : BindingFragment<FragmentCertificateViewBinding>() {

    private val args by navArgs<DccViewCertificateFragmentArgs>()
    ...
    override fun onCreateBinding(
        ...
        container: ViewGroup?
    ): FragmentCertificateViewBinding =
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.certificate.observe(viewLifecycleOwner) {
            ...
            binding.title.text = when {
                ...
                certificate.recoveryStatements?.isNotEmpty() == true -> binding.root.resources.getString(
                    R.string.recovery
                )
        ...
        binding.checkValidity.setOnClickListener {
            viewModel.certificate.value?.certificatesCard?.qrCodeText?.let {
                val action =
                    DccViewCertificateFragmentDirections.actionViewCertificateFragmentToCertificateValidityFragment(
                        it
                    )
        ...
        binding.revokedInfo.setOnClickListener {
            val action =
                DccViewCertificateFragmentDirections.actionViewCertificateFragmentToCertificateHasBeenRevokedDialogFragment()
            findNavController().navigate(action)
        }
    }
    ...
    private fun onViewModelEvent(event: ViewCertificateViewModel.ViewCertEvent) {
        when (event) {
            is ViewCertificateViewModel.ViewCertEvent.OnCertDeleted -> {
                requireActivity().let {
