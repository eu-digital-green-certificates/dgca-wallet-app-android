...
import com.android.app.dcc.databinding.FragmentCertificateClaimBinding
...
import dgca.wallet.app.android.dcc.utils.bindText
...
class ClaimCertificateFragment : BindingFragment<FragmentCertificateClaimBinding>() {
    ...
    override fun onCreateBinding(
        ...
        container: ViewGroup?
    ): FragmentCertificateClaimBinding =
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.recyclerView.adapter =
            CertListAdapter(
                layoutInflater,
                args.claimCertificateModel.certificateModel.getCertificateListData()
            )
        ...
        binding.saveBtn.setOnClickListener {
            val action =
                ClaimCertificateFragmentDirections.actionClaimCertificateFragmentToTanFragment()
        ...
        when (event) {
            is ClaimCertificateViewModel.ClaimCertEvent.OnCertClaimed -> {
                if (event.isClaimed) {
                    Toast.makeText(
                        requireContext(),
                        getString(R.string.certificate_claimed),
                        Toast.LENGTH_SHORT
                    ).show()
                    requireActivity().let {
            ...
            is ClaimCertificateViewModel.ClaimCertEvent.OnCertNotClaimed -> {
                Toast.makeText(
                    requireContext(),
                    getString(R.string.check_the_tan_and_try_again),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
