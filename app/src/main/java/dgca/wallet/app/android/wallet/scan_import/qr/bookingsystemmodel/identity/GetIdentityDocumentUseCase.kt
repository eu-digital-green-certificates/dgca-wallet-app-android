...
import dgca.wallet.app.android.data.remote.ticketing.VerificationMethodType
        ...
        ticketingApiService.getIdentity(identityUrl).body()?.let { identityResponse ->
            ...
            identityResponse.verificationMethods.forEach { verificationMethod ->
                when (verificationMethod.type) {
                    VerificationMethodType.ACCESS_TOKEN_SERVICE.type -> {
                    ...
                    VerificationMethodType.VALIDATION_SERVICE.type -> validationServices.add(ValidationService())
                    else -> {
